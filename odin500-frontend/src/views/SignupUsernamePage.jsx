...
import { getAuthToken, hasAuthSessionCookies } from '../store/apiStore.js';
...
function UsernameBody() {
  ...
  useEffect(() => {
    ...
    const syncToken = async () => {
      const cookieSession = await hasAuthSessionCookies();
      if (!cancelled) setHasToken(cookieSession || Boolean(getAuthToken()));
    };
    void syncToken();
    const onAuth = () => void syncToken();
  ...
  const onSubmit = useCallback(
    async (e) => {
      ...
      setError('');
      try {
        const sessionReady = await hasAuthSessionCookies();
        if (!sessionReady) {
          setError('Your session is not ready. Sign in once to save your display name.');
          return;
        }
        ...
        navigate('/virtual-portfolio/ai', { replace: true });
      } catch (err) {
